...
static char *fnbuf, *aprefix;

char *br_find_bin(char *file) {
	char *dir = br_find_bin_dir("/bin");
	snprintf(fnbuf, FILENAME_MAX, "%s/%s", dir, file);
	free(dir);
	return fnbuf;
}
char *br_find_sbin(char *file) {
	char *dir = br_find_sbin_dir("/sbin");
	snprintf(fnbuf, FILENAME_MAX, "%s/%s", dir, file);
	free(dir);
	return fnbuf;
}
char *br_find_data(char *file) {
	char *dir = br_find_data_dir("/usr/share");
	snprintf(fnbuf, FILENAME_MAX, "%s/%s/%s", dir, aprefix, file);
	free(dir);
	return fnbuf;
}
char *br_find_locale(char *file) {
	char *dir = br_find_locale_dir("/usr/share/locale");
	snprintf(fnbuf, FILENAME_MAX, "%s/%s", dir, file);
	free(dir);
	return fnbuf;
}
char *br_find_lib(char *file) {
	char *dir = br_find_lib_dir("/usr/lib");
	snprintf(fnbuf, FILENAME_MAX, "%s/%s", dir, file);
	free(dir);
	return fnbuf;
}
char *br_find_libexec(char *file) {
	char *dir = br_find_libexec_dir("/usr/libexec");
	snprintf(fnbuf, FILENAME_MAX, "%s/%s", dir, file);
	free(dir);
	return fnbuf;
}
char *br_find_etc(char *file) {
	char *dir = br_find_etc_dir("/etc");
	snprintf(fnbuf, FILENAME_MAX, "%s/%s/%s", dir, aprefix, file);
	free(dir);
	return fnbuf;
}
 ...
 */
int
br_init (BrInitError *error, char *aprefix0)
{
	exe = _br_find_exe (error);
	fnbuf = malloc(FILENAME_MAX);
	aprefix = aprefix0;
	return (exe != NULL) && (fnbuf != NULL);
}
